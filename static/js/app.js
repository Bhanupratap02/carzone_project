...
function adjustHeader() {
  var windowWidth = $(window).width();
  var defaultLogo = $(".company-logo img").data("default-logo");
  var shrinkLogo = $(".company-logo img").data("shrink-logo");
  ...
  if (windowWidth > 0) {
    if ($(document).scrollTop() >= 100) {
      ...
      if ($(".do-sticky").length < 1) {
        $(".company-logo img").attr("src", shrinkLogo);
      }
    } else {
      $(".sticky-header").removeClass("header-shrink");
      if (
        ...
        $(".company-logo img").attr("src", defaultLogo);
      } else {
        $(".company-logo img").attr("src", shrinkLogo);
      }
    }
